...
import { fetchIssues, parseRepoUrl, validateRepo } from "../services/octokit.js";
...
router.get("/analyze", requireAuth, async (req: Request, res: Response): Promise<void> => {
  ...
  try {
    ...
    if (refresh !== "true") {
      const cached = await Analysis.findOne({ owner, repo }).sort({ cachedAt: -1 });
    ...
    console.log(`✨ Generating merge suggestions for ${rawClusters.length} clusters...`);
